...
const handleLogin = async () => {
  ...
  try {
    const response = await fetch('/cvkuberkah/api/auth.php?action=login', {
      method: 'POST',
      headers: {
...
<template>
  <div class="auth-container">
    <div class="glass-card auth-card">
      <h2 class="auth-title">Masuk ke <span>CV Kuberkah</span></h2>
      <p class="auth-subtitle">Kelola dan unduh CV ATS profesional Anda.</p>
      ...
      <div class="auth-footer">
        Belum punya akun? <router-link :to="{ name: 'Register' }">Daftar di sini</router-link>
      </div>
    </div>
    ...
    <div v-if="showGoogleModal" class="modal-overlay">
      <div class="modal-card google-modal">
        ...
        <div class="google-custom-form">
          <h4>Atau masuk dengan akun Google lain:</h4>
          <div class="form-group">
